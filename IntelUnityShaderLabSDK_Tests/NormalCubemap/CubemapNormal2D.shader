Shader "Editor/CubemapNormal2D"
{
    Properties
    {
        ...
        _CubemapNormal2D("CubemapNormal2D", 2D) = "" {}
        _CubemapBump2D("CubemapBump2D", 2D) = "" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _CubemapNormal2D;
            fixed4 _CubemapNormal2D_ST;
            sampler2D _CubemapBump2D;
            fixed4 _CubemapBump2D_ST;
            ...
            appdata_cubemap vertex(appdata_full input)
            {
                ...
                //fixed4 bumpMap = tex2D(_CubemapBump2D, output.sample.xyz);
                fixed4 incorrectLODBumpMap = tex2Dlod(_CubemapBump2D, fixed4(((input.texcoord.xy * _CubemapBump2D_ST.xy) + _CubemapBump2D_ST.zw), 0.0, 0.0));
            ...
            fixed4 fragment(appdata_cubemap input) : COLOR
            {
                ...
                //fixed3 normalMap = tex2D(_CubemapNormal2D, input.sample) * 360.0;
                fixed3 incorrectNormalMap = tex2D(_CubemapNormal2D, (input.sample.xy * _CubemapNormal2D_ST.xy) + _CubemapNormal2D_ST.zw) * 360.0;
