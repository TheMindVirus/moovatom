Shader "Editor/CubemapNormal3D"
{
    Properties
    {
        ...
        _CubemapNormal3D("CubemapNormal3D", 3D) = "" {}
        _CubemapBump3D("CubemapBump3D", 3D) = "" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler3D _CubemapNormal3D;
            fixed4 _CubemapNormal3D_ST;
            sampler3D _CubemapBump3D;
            fixed4 _CubemapBump3D_ST;
            ...
            appdata_cubemap vertex(appdata_full input)
            {
                ...
                //fixed4 bumpMap = tex3D(_CubemapBump3D, output.sample.xyz);
                fixed4 incorrectLODBumpMap = tex3Dlod(_CubemapBump3D, fixed4((input.texcoord * _CubemapBump3D_ST).xyz, 0.0));
            ...
            fixed4 fragment(appdata_cubemap input) : COLOR
            {
                ...
                //fixed3 normalMap = tex3D(_CubemapNormal3D, input.sample) * 360.0;
                fixed3 incorrectNormalMap = tex3D(_CubemapNormal3D, input.sample * _CubemapNormal3D_ST) * 360.0;
